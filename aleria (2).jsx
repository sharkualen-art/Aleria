...
export default function Aleria(){
  ...
  // Load Firebase immediately on mount so it's ready when user clicks register/login
  useEffect(()=>{
    if(window.firebase_auth||document.getElementById("fb-app"))return;
    const s=document.createElement("script");
    s.id="fb-app";
    s.type="module";
    s.textContent=`
      import{initializeApp,getApps}from"https://www.gstatic.com/firebasejs/10.12.0/firebase-app.js";
      import{getAuth,createUserWithEmailAndPassword,signInWithEmailAndPassword,updateProfile,sendEmailVerification}from"https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js";
      const cfg={apiKey:"AIzaSyCwkGuv86j1u1eFhA75L1K4PVz4sg4f3iE",authDomain:"aleria-app.firebaseapp.com",projectId:"aleria-app"};
      const app=getApps().length?getApps()[0]:initializeApp(cfg);
      window.firebase_auth={auth:getAuth(app),createUserWithEmailAndPassword,signInWithEmailAndPassword,updateProfile,sendEmailVerification};
    ...
    if(onbStep==="login") return(
      <div style={wrap}>
        ...
        <button style={primaryBtn} disabled={form.authLoading} onClick={async()=>{
          ...
          try{
            const fb=window.firebase_auth;
            if(!fb)throw new Error("Firebase not ready. Please wait a moment and try again.");
